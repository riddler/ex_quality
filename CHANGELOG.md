...
- **A Doc links stage: fails on a relative link in the published Markdown that HexDocs or hex.pm would break, where ExDoc says nothing.** ExDoc rewrites a relative `.md` link only when its target is itself an extra, by basename alone, so any other relative link stays a raw `href` that works on GitHub and answers 404 on HexDocs, and a link whose basename matches another extra is rewritten to that extra silently; hex.pm renders the README from the tarball, so a README link answers 404 there unless the file ships. The stage reads `mix.exs`, the README and the Markdown extras and reports four rules as findings at the link's `file:line`: a README link or image the package files do not cover (Hex's default file list when `package` names no `files:`), a link in an extra to a file that is not an extra, two extras sharing a basename with no `filename:` on the second, and a link ExDoc would rewrite to a different extra. It builds nothing and runs no tool. **Opt-in, like Docs**, so nobody's gate changes on upgrade: enable with `doc_links: [enabled: :auto]` in `.quality.exs` (on when `:ex_doc` is installed), or `enabled: true` to force; `--skip-doc-links` skips it for a run
...
- **`format: [check: true]` makes the Format stage check formatting and fail on drift instead of rewriting files.** The write default is right for the interactive loop, but it means formatting can never fail a run anywhere: CI goes green on unformatted code and the next branch to touch those files inherits the reflow diff, and an unattended agent's gate quietly mutates the working tree mid-run. In check mode the stage runs `mix format --check-formatted` and stops there - a clean tree passes, drift fails the stage with the file list and nothing is written, and a syntax error is reported the same way the write mode reports it. The stage keeps the name `Format` in both modes so report consumers need not care; the stats key is the tell, `files_needing_format` in check mode against `files_formatted` in write mode, because "3 files were rewritten" and "3 files would be" are different claims. Default `check: false`: nobody's gate changes on upgrade
